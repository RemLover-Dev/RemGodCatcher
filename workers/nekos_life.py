...
def worker_nekos_life(category, amount, net_config):
    ...
    exclude_gif = net_config.get("nekos_life_exclude_gif", False)
    exclude_img = net_config.get("nekos_life_exclude_img", False)
    anti_ban_pause = float(net_config.get("anti_ban_pause", 3.0))

    if exclude_gif and category in GIF_ONLY:
        log_msg(name, f"'{category}' only returns GIFs — exclude GIFs enabled, nothing to download.")
        return
    if exclude_img and category in STATIC_ONLY:
        log_msg(name, f"'{category}' only returns static images — exclude images enabled, nothing to download.")
        return
    ...
    while not stop_event.is_set() and (amount == 0 or downloaded < amount):
        ...
        if exclude_gif and is_gif:
            continue
        if exclude_img and not is_gif:
            continue
